...
def generate_resumebook(request):
    ...
    for year in range(int(curr_year), int(curr_year) + 4):
        ...
        for u in by_year_users:
            by_year_names.update({
                f"{u['userInfo__user__last_name']}, {u['userInfo__user__first_name']}":
                    os.path.join(settings.MEDIA_ROOT, u['pdf'])
            })
    ...
    for year in year_range:
        title_contents.append((year, render_to_string("indrel/section_title.tex", {"year": year})))
    ...
    try:
        ...
        subprocess.run(
            ["pdflatex", "-output-directory", resumebook_dir, "-jobname", "indrel_letter"], 
        ...
        for title_content in title_contents:
            subprocess.run(
                ["pdflatex", "-output-directory", resumebook_dir, "-jobname", f"{title_content[0]}_Title"], 
        ...
        for year, _ in title_contents:
            title_pdf_paths.update({year: os.path.join(resumebook_dir, f"{year}_Title.pdf")})

        # Using PyPDF2 to merge the resumebook together
        merger = PdfWriter()
        
        cover_pdf_path = os.path.join(settings.MEDIA_ROOT, 'indrel', 'resumebook', 'cover.pdf')
        ...
        resumebook = HttpResponse(resumebook_pdf_content, content_type="application/pdf")
