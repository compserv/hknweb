...
from upeweb.utils import get_rand_photo, login_and_permission, method_login_and_permission
...
class CandRequestView(FormView, generic.ListView):
    ...
    def send_request_email(self, form):
        subject = '[UPE] Confirm Officer Challenge'
        ...
        msg = EmailMultiAlternatives(subject, subject,
                'no-reply@upe.cs.berkeley.edu', [officer_email])
...
def add_cands(request):
    ...
    for row in cand_csv:
        ...
        msg = EmailMultiAlternatives(subject, subject,
                "no-reply@upe.cs.berkeley.edu", [candidatedto.email])
        msg.attach_alternative(html_content, "text/html")
        msg.send()
...
class BitByteView(FormView, generic.ListView):
    ...
    def send_request_email(self, form):
        subject = '[UPE] Bit-byte request submitted'
