...
UPE_EMAIL_DOMAIN = "@upe.cs.berkeley.edu"
...
class CandidateDTO:
    def __init__(self, candidate_attributes: dict):
        ...
        elif self.email.endswith(UPE_EMAIL_DOMAIN):
            self.username = self.email.replace(UPE_EMAIL_DOMAIN, "")
        self.validate()

    def validate(self):
        assert self.email, "Candidate email must not be empty"
        assert self.email.endswith(BERKELEY_EMAIL_DOMAIN) or self.email.endswith(UPE_EMAIL_DOMAIN), "Candidate email must be an @berkeley.edu email"
